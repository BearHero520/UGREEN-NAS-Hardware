                                         ...
                                         char *error, size_t error_size)
{
    ...
    (void)snprintf(error, error_size,
                   "DX4600 family writes are firmware-reversed but await physical validation; "
                   "use --force together with --apply on an exact DX4600 product");
    return -EPERM;
}
...
{
    (void)request;
    return it8613_dx4600_read_fan(fans, fan_count, error, error_size);
}
...
{
    (void)request;
    return dx4600_read_wol_policy(policy, error, error_size);
}
                       ...
                       char *error, size_t error_size)
{
    ...
    return it8613_dx4600_set_fan_pwm(fan_id, pwm, error, error_size);
}
                          ...
                          char *error, size_t error_size)
{
    ...
    return dx4600_set_wol_policy(policy, error, error_size);
}
...
const struct ugreenctl_plugin *ugreenctl_plugin_v5(void)
{
    static const struct ugreenctl_plugin plugin = {
        ...
        .id = "dx4600",
        .display_name = "UGREEN DX4600 / DX4600+ / DX4600 Pro (firmware-reversed)",
