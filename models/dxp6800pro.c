                                         ...
                                         char *error, size_t error_size)
{
    ...
    (void)snprintf(error, error_size,
                   "DXP6800 Pro writes are firmware-reversed but await physical validation; "
                   "use --force together with --apply on an exact DXP6800 Pro");
    return -EPERM;
}
...
{
    (void)request;
    return it8613_dxp6800pro_read_fans(fans, fan_count, error, error_size);
}
...
{
    (void)request;
    return dxp6800pro_read_wol_policy(policy, error, error_size);
}
                       ...
                       char *error, size_t error_size)
{
    ...
    return it8613_dxp6800pro_set_fan_pwm(true, fan_id, pwm, error, error_size);
}
                          ...
                          char *error, size_t error_size)
{
    ...
    return dxp6800pro_set_wol_policy(policy, error, error_size);
}
...
const struct ugreenctl_plugin *ugreenctl_plugin_v5(void)
{
    static const struct ugreenctl_plugin plugin = {
        ...
        .id = "dxp6800pro",
        .display_name = "UGREEN DXP6800 Pro (firmware-reversed)",
